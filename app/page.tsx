...
export default function Home() {
  ...
  const [registeredPersonalActions, setRegisteredPersonalActions] = useState<any[]>([]);
  const [userGoal, setUserGoal] = useState<{title: string, startDate: string, durationDays: number} | null>(null);

  const currentGoal = userGoal || goal;
  const { dayIndex, progressPct } = calculateProgress(currentGoal.startDate, currentGoal.durationDays);
  ...
  const currentActions = registeredPersonalActions.length > 0 ? registeredPersonalActions : actionsCompany;
  ...
  const handleLogActions = () => {
    ...
    const now = new Date();
    const timeStr = now.toLocaleDateString('ja-JP', { month: 'numeric', day: 'numeric' });
  ...
  const handleReset = () => {
    if (confirm('目標とアクションをすべて削除して、再設定しますか？')) {
      ...
      setToast('目標・アクションをリセットしました。目標・アクションページで再設定してください。');
    }
  };
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="bg-white rounded-2xl shadow p-6 space-y-4">
          ...
          {registeredPersonalActions.length > 0 ? (
            <div>
              <h2 className="text-xl font-bold text-gray-900 mb-2">{currentGoal.title}</h2>
            ...
            <div className="bg-blue-50 border-2 border-blue-300 rounded-lg p-4">
              <h3 className="text-lg font-bold text-blue-900 mb-2">目標・アクションを設定しましょう</h3>
          ...
          {registeredPersonalActions.length > 0 && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-3">
              <p className="text-sm text-green-800 font-medium">
                ✓ 登録したアクション（{registeredPersonalActions.length}件）を使用中
              </p>
        ...
        <div className="bg-white rounded-2xl shadow p-6 space-y-4">
          <h2 className="text-lg font-bold text-gray-900">週次アクション振り返り</h2>
          ...
          <div className="space-y-3">
            ...
            >
              {selectedActionIds.length > 0 ? `${selectedActionIds.length}件を登録` : '登録'}
            </button>

            {todayLogs.length > 0 && (
              <div className="pt-4 border-t border-gray-200 mt-4">
                <p className="text-sm font-medium text-gray-700 mb-3">今週の登録アクション ({todayLogs.length}件)</p>
                <div className="space-y-2">
